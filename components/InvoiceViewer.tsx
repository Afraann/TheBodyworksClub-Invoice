...
export function InvoiceViewer({ invoice }: InvoiceViewerProps) {
  ...
  return (
    <div className="min-h-screen bg-neutral-100 flex flex-col items-center py-6 px-4 gap-6 font-sans text-neutral-900">
      ...
      <div className="w-full max-w-4xl bg-neutral-900 text-white p-4 rounded-xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 print:hidden">
        ...
        <div className="flex flex-wrap justify-center gap-3 w-full md:w-auto">
             ...
             <button 
                ...
                className="flex items-center gap-2 px-5 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-bold uppercase tracking-wide shadow-lg shadow-red-900/20 transition-all"
             >
      ...
      {/* 2. PREVIEW AREA (Mobile Scrolling Fix) */}
      <div className="w-full flex-1 overflow-auto flex justify-center items-start pb-20">
          
          <div 
            ref={componentRef}
            id="invoice-preview"
            className={`bg-white shadow-2xl overflow-hidden print:shadow-none transition-all duration-300
                ${printMode === 'THERMAL' 
                    ? 'w-[80mm] min-h-[100mm] text-[10px] font-mono p-2 pb-10' 
                    : 'w-[210mm] h-[297mm] min-w-[210mm] relative' // Fixed width for A4, forces scroll on mobile
                }
            `}
          >
            {printMode === 'A4' ? (
                ...
                <>
                    ...
                    {/* Customer Info */}
                    <div className="absolute z-10 top-[24%] left-[22%] font-bold text-neutral-800 uppercase tracking-wide text-sm">
                        {invoice.customerName}
                    </div>
                    <div className="absolute z-10 top-[26.5%] left-[25%] font-bold text-neutral-800 font-mono text-sm">
                        {invoice.customerPhone}
                    </div>

                    {/* Invoice Details */}
                    <div className="absolute z-10 top-[24%] left-[75%] font-bold text-neutral-800 text-sm">
                        {invoice.invoiceCode}
                    </div>
                    <div className="absolute z-10 top-[26.2%] left-[75%] font-bold text-neutral-800 text-sm">
                        {new Date(invoice.invoiceDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </div>
                    <div className="absolute z-10 top-[28.74%] left-[75%] font-bold text-neutral-800 text-sm">
                        {branch?.gstin || 'N/A'}
                    </div>
                            ...
                            <tbody>
                                {items.map((item: any) => (
                                    <tr key={item.id} className="h-8 align-top">
                                        <td className="w-[45%] pl-2 font-bold text-neutral-800 uppercase text-xs">
                                            {item.description}
                                        </td>
                                        <td className="w-[30%] pl-8 font-bold text-neutral-600 text-xs">
                                            {item.durationDays ? `${item.durationDays} Days` : '-'}
                                        </td>
                                        <td className="w-[25%] text-right pr-4 font-bold text-neutral-900 text-sm">
